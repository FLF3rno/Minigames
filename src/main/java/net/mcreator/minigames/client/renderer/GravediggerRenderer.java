...
public class GravediggerRenderer extends MobRenderer<GravediggerEntity, LivingEntityRenderState, Modelgravedigger> {
	...
	public GravediggerRenderer(EntityRendererProvider.Context context) {
		...
		this.addLayer(new RenderLayer<>(this) {
			...
			public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int light, LivingEntityRenderState state, float headYaw, float headPitch) {
				EntityModel model = new Modelgravedigger(Minecraft.getInstance().getEntityModels().bakeLayer(Modelgravedigger.LAYER_LOCATION));
	...
	public Identifier getTextureLocation(LivingEntityRenderState state) {
		GravediggerEntity entity = (GravediggerEntity) state.getRenderData(ENTITY_KEY);
	...
	private static final class AnimatedModel extends Modelgravedigger {
		...
		public void setupAnim(LivingEntityRenderState state) {
			...
			GravediggerEntity entity = state.getRenderData(ENTITY_KEY);
	...
	public static final ContextKey<GravediggerEntity> ENTITY_KEY = new ContextKey<>(Identifier.parse("minigames:gravedigger_entity"));

	@EventBusSubscriber(Dist.CLIENT)
	public static class EntityStateAdder {
		@SubscribeEvent
		private static void registerRenderStateModifiersEvent(RegisterRenderStateModifiersEvent event) {
			event.registerEntityModifier(GravediggerRenderer.class, (entity, state) -> state.setRenderData(ENTITY_KEY, entity));
		}
	}
}