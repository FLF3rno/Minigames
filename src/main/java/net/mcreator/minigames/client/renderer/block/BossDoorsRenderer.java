...
import net.mcreator.minigames.block.entity.BossDoorsBlockEntity;
import net.mcreator.minigames.block.BossDoorsBlock;
...
public class BossDoorsRenderer implements BlockEntityRenderer<BossDoorsBlockEntity, BossDoorsRenderer.CustomRenderState> {
	private final CustomHierarchicalModel model;
	private final Identifier texture;

	BossDoorsRenderer(BlockEntityRendererProvider.Context context) {
		this.model = new CustomHierarchicalModel(context.bakeLayer(Modelhalfblock.LAYER_LOCATION));
		this.texture = Identifier.parse("minigames:textures/block/bossdoor.png");
	}
	...
	@Override
	public void extractRenderState(BossDoorsBlockEntity blockEntity, CustomRenderState state, float partialTicks, Vec3 cameraPosition, ModelFeatureRenderer.CrumblingOverlay breakProgress) {
	...
	public void submit(CustomRenderState renderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
		...
		Direction facing = state.getValue(BossDoorsBlock.FACING);
		switch (facing) {
			case NORTH -> {
			}
	...
	public static void registerBlockEntityRenderers(EntityRenderersEvent.RegisterRenderers event) {
		event.registerBlockEntityRenderer(MinigamesModBlockEntities.BOSS_DOORS.get(), BossDoorsRenderer::new);
	}
	...
	public static class CustomRenderState extends BlockEntityRenderState {
		...
		protected BossDoorsBlockEntity blockEntity;
	...
	private static final class CustomHierarchicalModel extends Modelhalfblock {
		...
		public void setupBlockEntityAnim(BossDoorsBlockEntity blockEntity, LivingEntityRenderState state) {
