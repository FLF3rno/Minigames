...
import net.mcreator.minigames.block.entity.FloorDoorsBlockEntity;
import net.mcreator.minigames.block.FloorDoorsBlock;
...
public class FloorDoorsRenderer implements BlockEntityRenderer<FloorDoorsBlockEntity, FloorDoorsRenderer.CustomRenderState> {
	private final CustomHierarchicalModel model;
	private final Identifier texture;

	FloorDoorsRenderer(BlockEntityRendererProvider.Context context) {
		this.model = new CustomHierarchicalModel(context.bakeLayer(Modelhalfblock.LAYER_LOCATION));
		this.texture = Identifier.parse("minigames:textures/block/floordoor.png");
	}
	...
	@Override
	public void extractRenderState(FloorDoorsBlockEntity blockEntity, CustomRenderState state, float partialTicks, Vec3 cameraPosition, ModelFeatureRenderer.CrumblingOverlay breakProgress) {
	...
	public void submit(CustomRenderState renderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
		...
		Direction facing = state.getValue(FloorDoorsBlock.FACING);
		switch (facing) {
			case NORTH -> {
			}
	...
	public static void registerBlockEntityRenderers(EntityRenderersEvent.RegisterRenderers event) {
		event.registerBlockEntityRenderer(MinigamesModBlockEntities.FLOOR_DOORS.get(), FloorDoorsRenderer::new);
	}
	...
	public static class CustomRenderState extends BlockEntityRenderState {
		...
		protected FloorDoorsBlockEntity blockEntity;
	...
	private static final class CustomHierarchicalModel extends Modelhalfblock {
		...
		public void setupBlockEntityAnim(FloorDoorsBlockEntity blockEntity, LivingEntityRenderState state) {
