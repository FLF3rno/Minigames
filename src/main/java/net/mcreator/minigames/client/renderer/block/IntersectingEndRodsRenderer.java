...
public class IntersectingEndRodsRenderer implements BlockEntityRenderer<IntersectingEndRodsBlockEntity> {
	...
	IntersectingEndRodsRenderer(BlockEntityRendererProvider.Context context) {
		...
		this.texture = ResourceLocation.parse("minigames:textures/block/intersecting_end_rod.png");
		this.renderState = new LivingEntityRenderState();
	}

	private void updateRenderState(IntersectingEndRodsBlockEntity blockEntity, float partialTick) {
		int tickCount = (int) blockEntity.getLevel().getGameTime();
		renderState.ageInTicks = tickCount + partialTick;
	}

	@Override
	public void render(IntersectingEndRodsBlockEntity blockEntity, float partialTick, PoseStack poseStack, MultiBufferSource renderer, int light, int overlayLight, Vec3 cameraPos) {
