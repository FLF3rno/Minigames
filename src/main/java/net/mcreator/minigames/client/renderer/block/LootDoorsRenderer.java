...
import net.mcreator.minigames.block.entity.LootDoorsBlockEntity;
import net.mcreator.minigames.block.LootDoorsBlock;
...
public class LootDoorsRenderer implements BlockEntityRenderer<LootDoorsBlockEntity, LootDoorsRenderer.CustomRenderState> {
	private final CustomHierarchicalModel model;
	private final Identifier texture;

	LootDoorsRenderer(BlockEntityRendererProvider.Context context) {
		this.model = new CustomHierarchicalModel(context.bakeLayer(Modelhalfblock.LAYER_LOCATION));
		this.texture = Identifier.parse("minigames:textures/block/lootdoor.png");
	}
	...
	@Override
	public void extractRenderState(LootDoorsBlockEntity blockEntity, CustomRenderState state, float partialTicks, Vec3 cameraPosition, ModelFeatureRenderer.CrumblingOverlay breakProgress) {
	...
	public void submit(CustomRenderState renderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
		...
		Direction facing = state.getValue(LootDoorsBlock.FACING);
		switch (facing) {
			case NORTH -> {
			}
	...
	public static void registerBlockEntityRenderers(EntityRenderersEvent.RegisterRenderers event) {
		event.registerBlockEntityRenderer(MinigamesModBlockEntities.LOOT_DOORS.get(), LootDoorsRenderer::new);
	}
	...
	public static class CustomRenderState extends BlockEntityRenderState {
		...
		protected LootDoorsBlockEntity blockEntity;
	...
	private static final class CustomHierarchicalModel extends Modelhalfblock {
		...
		public void setupBlockEntityAnim(LootDoorsBlockEntity blockEntity, LivingEntityRenderState state) {
