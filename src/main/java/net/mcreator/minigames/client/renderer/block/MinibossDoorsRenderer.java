...
import net.mcreator.minigames.block.entity.MinibossDoorsBlockEntity;
import net.mcreator.minigames.block.MinibossDoorsBlock;
...
public class MinibossDoorsRenderer implements BlockEntityRenderer<MinibossDoorsBlockEntity, MinibossDoorsRenderer.CustomRenderState> {
	private final CustomHierarchicalModel model;
	private final Identifier texture;

	MinibossDoorsRenderer(BlockEntityRendererProvider.Context context) {
		this.model = new CustomHierarchicalModel(context.bakeLayer(Modelhalfblock.LAYER_LOCATION));
		this.texture = Identifier.parse("minigames:textures/block/minibossdoor.png");
	}
	...
	@Override
	public void extractRenderState(MinibossDoorsBlockEntity blockEntity, CustomRenderState state, float partialTicks, Vec3 cameraPosition, ModelFeatureRenderer.CrumblingOverlay breakProgress) {
	...
	public void submit(CustomRenderState renderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
		...
		Direction facing = state.getValue(MinibossDoorsBlock.FACING);
		switch (facing) {
			case NORTH -> {
			}
	...
	public static void registerBlockEntityRenderers(EntityRenderersEvent.RegisterRenderers event) {
		event.registerBlockEntityRenderer(MinigamesModBlockEntities.MINIBOSS_DOORS.get(), MinibossDoorsRenderer::new);
	}
	...
	public static class CustomRenderState extends BlockEntityRenderState {
		...
		protected MinibossDoorsBlockEntity blockEntity;
	...
	private static final class CustomHierarchicalModel extends Modelhalfblock {
		...
		public void setupBlockEntityAnim(MinibossDoorsBlockEntity blockEntity, LivingEntityRenderState state) {
