...
import net.mcreator.minigames.block.entity.SprucePewBlockEntity;
import net.mcreator.minigames.block.SprucePewBlock;
...
public class SprucePewRenderer implements BlockEntityRenderer<SprucePewBlockEntity, SprucePewRenderer.CustomRenderState> {
	private final CustomHierarchicalModel model;
	private final Identifier texture;

	SprucePewRenderer(BlockEntityRendererProvider.Context context) {
		this.model = new CustomHierarchicalModel(context.bakeLayer(Modelpewseat.LAYER_LOCATION));
	...
	@Override
	public void extractRenderState(SprucePewBlockEntity blockEntity, CustomRenderState state, float partialTicks, Vec3 cameraPosition, ModelFeatureRenderer.CrumblingOverlay breakProgress) {
	...
	public void submit(CustomRenderState renderState, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
		...
		Direction facing = state.getValue(SprucePewBlock.FACING);
		switch (facing) {
			case NORTH -> {
			}
	...
	public static void registerBlockEntityRenderers(EntityRenderersEvent.RegisterRenderers event) {
		event.registerBlockEntityRenderer(MinigamesModBlockEntities.SPRUCE_PEW.get(), SprucePewRenderer::new);
	}
	...
	public static class CustomRenderState extends BlockEntityRenderState {
		...
		protected SprucePewBlockEntity blockEntity;
		protected BlockState blockState;
	}

	private static final class CustomHierarchicalModel extends Modelpewseat {
		...
		public void setupBlockEntityAnim(SprucePewBlockEntity blockEntity, LivingEntityRenderState state) {
