...
public class AchievementHudOverlay {
	private static final Identifier IMAGE_0 = Identifier.parse("minigames:textures/screens/thunder.png");
	...
	public static void eventHandler(RenderGuiEvent.Pre event) {
		...
		if (ShowAchievementOverlayProcedure.execute(world)) {
			if (ShowMod2Procedure.execute(world)) {
				event.getGuiGraphics().blit(RenderPipelines.GUI_TEXTURED, IMAGE_0, w / 2 + -169, h - 38, 0, 0, 48, 48, 48, 48);
			}
			...
			if (HidePVPIconHuntProcedure.execute(world)) {
				event.getGuiGraphics().blit(RenderPipelines.GUI_TEXTURED, SPRITE_1, 2, h - 45, 0, Mth.clamp((int) PVPDisplayProcedure.execute(world, entity) * 50, 0, 100), 50, 50, 50, 150);
			}
			if (ShowMod1Procedure.execute(world)) {
				event.getGuiGraphics().blit(RenderPipelines.GUI_TEXTURED, SPRITE_2, 42, h - 47, 0, 200, 50, 50, 50, 250);
			}
