...
public class BeamScreenTintOverlay {
    ...
    private static final int BEAM_COLOR_RGB = 0xFFFF45;
    private static final int MAX_ALPHA = 145;
    ...
    public static void onRenderGui(RenderGuiEvent.Post event) {
        ...
        event.getGuiGraphics().fill(
                ...
                width,
                height,
                (alpha << 24) | BEAM_COLOR_RGB
        );
    }

    private static float getBeamIntensity(
            Vec3 point,
        ...
        for (Player player : players) {
            ...
            ModDataAttachments.BeamData data =
                    player.getData(ModDataAttachments.BEAM_DATA);
            ...
            strongest = Math.max(
                    strongest,
                    computeInsideStrength(point, data)
            );
    ...
    private static float computeInsideStrength(
            Vec3 point,
            ModDataAttachments.BeamData data
    ) {
        ...
        double proximity = 1.0 - (
                dist / Math.max(radius, 0.001)
        );

        double minStrength = 0.55;

        return (float) Mth.clamp(
                minStrength + proximity * (1.0 - minStrength),
                0.0,
                1.0
        );
    }
