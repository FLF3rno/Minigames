	...
	public void setupAnim(PlayerRenderState renderState, CallbackInfo ci) {
		...
		if (!master.equals("minigames")) {
			if (!MinigamesModPlayerAnimationAPI.animations.isEmpty())
				MinigamesModPlayerAnimationAPI.animations.clear();
			return;
		}
		Player player = (Player) renderState.getRenderData(MinigamesModPlayerAnimationAPI.ClientAttachments.PLAYER);
		if (player == null)
		...
		if (data.getBooleanOr("ResetPlayerAnimation", false)) {
			...
			MinigamesModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		MinigamesModPlayerAnimationAPI.PlayerAnimation animation = MinigamesModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = MinigamesModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				MinigamesMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			MinigamesModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, MinigamesModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			MinigamesModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
			ModelPart modelPart = getModelPart(model, boneName);
			if (modelPart == null)
				continue;
			// Apply rotation
			Vec3 rotation = MinigamesModPlayerAnimationAPI.PlayerBone.interpolate(bone.rotations, animationProgress, player);
			if (rotation != null) {
			...
			Vec3 position = MinigamesModPlayerAnimationAPI.PlayerBone.interpolate(bone.positions, animationProgress, player);
			if (position != null) {
			...
			// Apply scale
			Vec3 scale = MinigamesModPlayerAnimationAPI.PlayerBone.interpolate(bone.scales, animationProgress, player);
			if (scale != null) {
