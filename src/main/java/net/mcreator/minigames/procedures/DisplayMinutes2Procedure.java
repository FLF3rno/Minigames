...
public class DisplayMinutes2Procedure {
	public static double execute(LevelAccessor world) {
		if (((MinigamesModVariables.MapVariables.get(world).gameMinutes + "").replace(".0", "")).length() == 2) {
			return parseDouble((new java.text.DecimalFormat("##").format(MinigamesModVariables.MapVariables.get(world).gameMinutes)).substring(1, 2));
		}
