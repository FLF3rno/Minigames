		...
		for (Entity entityiterator : new ArrayList<>(world.players())) {
			if (entityiterator instanceof Player || entityiterator instanceof ServerPlayer) {
				if (entityiterator.getData(MinigamesModVariables.PLAYER_VARIABLES).team == 1) {
					if ((entityiterator.level().dimension()) == ResourceKey.create(Registries.DIMENSION, ResourceLocation.parse("minigames:dungeon_dimension"))) {
						world.setBlock(
								BlockPos.containing(x, y, z), BuiltInRegistries.BLOCK
