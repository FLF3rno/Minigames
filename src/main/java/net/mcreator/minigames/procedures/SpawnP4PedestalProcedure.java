...
public class SpawnP4PedestalProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (MinigamesModVariables.MapVariables.get(world).SpawnItems) {
			if ((world instanceof Level _lvl ? _lvl.dimension() : (world instanceof WorldGenLevel _wgl ? _wgl.getLevel().dimension() : Level.OVERWORLD)) == ResourceKey.create(Registries.DIMENSION, Identifier.parse("minigames:dungeon_dimension"))) {
				if ((world.isClientSide() ? Minecraft.getInstance().getConnection().getOnlinePlayers().size() : ServerLifecycleHooks.getCurrentServer().getPlayerCount()) < 4) {
					world.setBlock(BlockPos.containing(x, y, z), Blocks.AIR.defaultBlockState(), 3);
				} else {
					for (Entity entityiterator : new ArrayList<>(world.players())) {
						if (entityiterator instanceof Player || entityiterator instanceof ServerPlayer) {
							if (entityiterator.getData(MinigamesModVariables.PLAYER_VARIABLES).team == 4) {
								{
									...
									if (_bs.getBlock().getStateDefinition().getProperty("owner") instanceof IntegerProperty ownerProperty
											&& ownerProperty.getPossibleValues().contains(4))
										_bs = _bs.setValue(ownerProperty, 4);
