	...
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		for (Entity entityiterator : new ArrayList<>(world.players())) {
			if (!MinigamesModVariables.MapVariables.get(world).achievementHunterMode) {
				...
				MinigamesModVariables.MapVariables.get(world).hunterAnimation = 0;
				...
				MinigamesMod.queueServerWork(160, () -> {
					...
					if (entityiterator instanceof ServerPlayer _ent) {
						...
						_ent.openMenu(new MenuProvider() {
							@Override
							public Component getDisplayName() {
								return Component.literal("DisplayAchievmen");
							}
							...
							@Override
							public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
								return new DisplayAchievmenMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(_bpos));
							}
						}, _bpos);
					}
				});
